...
v Brně. Doménou modelu je schopnost simulovat zátěž sítě nabíjecích stanic na základě zájmu o využítí 
...
v roce 2030. K realizaci modulu byly využity jak veřejná data - viz. kapitola \ref{sec:zdroje} a neveřejná data, 
kde konkrétní data ani zdroje nemohou být publikovány. Tyto informace byly použity k vytvoření představy 
...
obsahuje seznam všech veřejných nabíjecích stanic společnosti ČEZ, dalším poskytovatelem je organizace PRE (Pražská energetika),
list stanic této skupiny lze najít na stránkách \href{https://chargepre.smatrics.com/cs/}{https://chargepre.smatrics.com/cs/}, mezi další velké hráče na poli elektromobility v Brně patří společnosti
...
celkově 305. \cite{eon_data, emobilita_data, pre_stations}
...
\subsection{Rozšíření modelu\\}
Pro další experimenty byl model rozšířen o nové typy nabíjecích stanic, byly přidány nabíječky s výkonem 80~kW a 160~kW, 
podle nejaktuálnějších dat. Tyto nabíječky byly zahrnuty do experimentů s aktuálním stavem celého Brněnského ekosystému.
Jejich kompletní výsledky si můžete prohlédnout v podadresáři \texttt{output\_v2/}.

\subsection{Experimenty s rozšířením\\}
\paragraph{Celodenní režim s rozšířenou infrastrukturou:}
Tento experiment vylepšuje experiment číslo \ref{dataset-processing}, model byl vylepšen o 80kWh a 160kWh nabíječky. Při aktuálním stavu systému, výsledky modelu ukazují, že
při standardních počtěch nabíjecích stanic v Brně (viz. tabulka \ref{table:charging-stations-distribution-improved}), přijelo za celý den k nabíjecím stanicím 310 elektromobilů,
z toho 307 vozidel dokončilo nabíjení. Průměčná doba nabíjení dosáhla hodnoty 107.66 minut a medián doby nabíjení činí 53.2952 se směrodatnou
odchylkou 42.9524 minut. Lze tedy vyvodit stejný závěr jako v předchozí simulací aktuálního stavu, že za současné situace 
je aktuální systém dostačující.
...
\begin{table}[h!]
    ...
    \begin{tabular}{|c|c|c|c|}
        ...
        50 kW  DC &  50 kW   & stejnosměrný proud   & 54  \\
        \hline
        80 kW DC  &  80 kW   & stejnosměrný proud   & 4  \\
        \hline
        108 kW DC &  108 kW  & stejnosměrný proud   & 11  \\
        \hline
        150 kW DC &  150 kW  & stejnosměrný proud   & 13  \\
        \hline
        160 kW DC &  160 kW  & stejnosměrný proud   & 4  \\
        \hline
    \end{tabular}
    \caption{Rozšířené rozdělení typu veřejných nabíjecích stanic v Brně podle výkonu a typu proudu}
    \label{table:charging-stations-distribution-improved}
    \vspace{0.5cm} % Space after table
\end{table}

\subsection{Rozšířená Petriho síť}
\begin{figure}[H]
    \centering
    \scalebox{0.095}{
        \includegraphics{ims-petri-improved.eps}
    }
    \caption{Upravená Petriho síť elektromobility v Brně}
    \label{figure:ims-petri-net-improved}
\end{figure}

\newpage

\paragraph{Celodenní režim (\texttt{24h}) s rozšířenou infrastrukturou o 80kWh a 160kWh nabíječky a odhadovaným budoucím zájmem o nabíjení:}
V předchozím experimentu jsme tedy dokázali, že aktuální infrastruktura vyhovuje poptávce, jak ale bude vypadat situace, kdy 
infrastruktura se nebude rozvíjet -- tedy zůstane na současných hodnotách, ale poptávka po nabíjení se zvýší? Pojďme se opět podívat 
do roku 2030, kdy se očekává, že počet elektromobilů se v tomto roce může pohybovat v okolí Brna kolem 20 tisíc. Předpokládejme, že
zájem o nabíjení na veřejných stanicích se zvýší na 25\%. Model této situace byl spuštěn s následujícím příkazem:

\begin{center}
    \texttt{./sim 24h -tc 20000 -p 0.25}
\end{center}

Výsledky ukazují, že v tomto scénáři bylo vygenerováno 5599 elektromobilů, z nichž 5476 zahájilo nabíjení a pouze 5404 automobilů bylo nabito, což odpovídá přibližně 96,5 \% 
všech generovaných elektromobilů. Průměrná doba nabíjení činila 105.66 minut. Co je, ale více zarážející je medián s hodnotou 108.318 minut se směrodatnou odchylkou 110 minut, 
což je více než dvojnásobek aktuálního stavu, jež naznačuje, že bude třeba do budoucna infrastrukturu značně rozšířit.
